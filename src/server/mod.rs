...
fn generate_routes() -> Router {
    let router = Router::new().route("/:name", post(post_handler));

    router
}

async fn post_handler(Path(name): Path<String>) -> impl IntoResponse {
    // Do something with the dynamic name parameter
    format!("Hello, {}!\n", name)
}
